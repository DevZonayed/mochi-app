...
const EFFORT_STOPS = ['FAST', 'BALANCED', 'DEEP', 'MAX'];
const EFFORT_META = {
...
function StrengthBars({ level = 2, tint = 'var(--blue)', size = 15 }) {
...
function EffortDial({ value = 'BALANCED', compact, onChange }) {
...
function ProviderGlyph({ provider, size = 18 }) {
  if (provider === 'anthropic' && window.AnthropicGlyph) return <AnthropicGlyph size={size} />;
  if (provider === 'openai' && window.OpenAIGlyph) return <OpenAIGlyph size={size} />;
...
function ModelSwitcher({ value = 'auto', onChange, models, compact, align = 'left' }) {
    ...
    <span ref={ref} style={{ position: 'relative', display: 'inline-flex' }}>
      <button onClick={() => interactive && setOpen(o => !o)} disabled={!interactive}
        className="model-btn" style={{
          ...
          borderRadius: 'var(--r-md, 8px)', borderRadius: 9, background: 'var(--fill-secondary)', color: 'var(--ink)',
...
Object.assign(window, { PillButton, GroupedList, Row, StatusPill, Spinner, Switch, EffortDial, StrengthBars, ModelSwitcher, DEFAULT_MODELS, EFFORT_EST });
