/* Pair a Phone — centered glass card morphing through pairing states. */

const PAIR_W = 1100, PAIR_H = 720;

function QRTile({ size = 200 }) {
  const N = 25, cell = size / N;
  let s = 4242; const rnd = () => { s = (s * 1103515245 + 12345) & 0x7fffffff; return s / 0x7fffffff; };
  const finder = (r, c) => { const b = (br, bc) => r >= br && r < br + 7 && c >= bc && c < bc + 7; return b(0, 0) || b(0, N - 7) || b(N - 7, 0); };
  const cells = [];
  for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) { if (finder(r, c)) continue; if (rnd() > 0.5) cells.push(<rect key={`${r}-${c}`} x={c * cell} y={r * cell} width={cell} height={cell} rx={cell * 0.2} />); }
  const F = (x, y) => <g><rect x={x} y={y} width={cell * 7} height={cell * 7} rx={cell * 1.6} fill="none" stroke="#000" strokeWidth={cell} /><rect x={x + cell * 2} y={y + cell * 2} width={cell * 3} height={cell * 3} rx={cell * 0.8} fill="#000" /></g>;
...
function PairCard({ state, secs, onRegen, onState }) {
  const total = 120;
      ...
      {state === 'waiting' && (
        <React.Fragment>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, margin: '24px 0 18px' }}>
            {['Open Maestro', 'Tap Pair', 'Scan'].map((s, i) => (
              <React.Fragment key={i}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, font: '500 var(--fs-footnote)/1 var(--font-text)', color: 'var(--ink-secondary)' }}>
                  <span style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--fill-secondary)', display: 'grid', placeItems: 'center', font: '700 var(--fs-caption)/1 var(--font-mono)', color: 'var(--ink-tertiary)' }}>{i + 1}</span>{s}
                </span>
              </React.Fragment>
            ))}
          </div>
...
function PairPhone() {
  ...
  React.useEffect(() => {
    ...
    const t = setInterval(() => setSecs(s => { if (s <= 1) { clearInterval(t); setState('expired'); return 0; } return s - 1; }), 1000);
    ...
    <div style={{ width: '100vw', height: '100vh', display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
      <div style={{ width: PAIR_W, height: PAIR_H, transform: `scale(${scale})`, transformOrigin: 'center', borderRadius: 18, overflow: 'hidden', position: 'relative', background: 'var(--backdrop-base)', boxShadow: '0 0 0 0.5px rgba(0,0,0,0.18), 0 40px 100px rgba(10,15,40,0.45)' }}>
        ...
        <div style={{ position: 'absolute', top: 16, left: '50%', transform: 'translateX(-50%)', zIndex: 30, display: 'inline-flex', padding: 3, borderRadius: 'var(--r-pill)', background: 'var(--glass-tint)', border: '0.5px solid var(--glass-border)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
          {[['waiting', 'Waiting'], ['detected', 'Detected'], ['paired', 'Paired'], ['expired', 'Expired'], ['error', 'Error']].map(([k, l]) => (
        ...
        <div style={{ position: 'absolute', top: 16, right: 18, zIndex: 30, display: 'flex', gap: 2, padding: 3, borderRadius: 'var(--r-pill)', background: 'var(--glass-tint)', border: '0.5px solid var(--glass-border)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
          {[['light', 'sun'], ['dark', 'moon']].map(([t, ic]) => <button key={t} onClick={() => setTheme(t)} style={{ width: 30, height: 26, borderRadius: 'var(--r-pill)', display: 'grid', placeItems: 'center', background: theme === t ? 'var(--bg-elevated)' : 'transparent', color: theme === t ? 'var(--ink)' : 'var(--on-glass)' }}><Icon name={ic} size={15} /></button>)}
        </div>
...
ReactDOM.createRoot(document.getElementById('root')).render(<PairPhone />);
