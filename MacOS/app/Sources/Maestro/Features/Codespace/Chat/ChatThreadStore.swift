...
final class ChatThreadStore {
    ...
    /// Switch the thread to a session (nil = a fresh, not-yet-created chat).
    func bind(_ sessionId: String?) async {
        self.sessionId = sessionId
        turns = []
        guard let sid = sessionId else { return }
        loading = true
        do { turns = try await client.call("listJobs", ["projectId": projectId, "sessionId": sid], as: [Job].self).sorted { $0.createdAt < $1.createdAt } }
        catch { turns = [] }
        loading = false
    }
    ...
    func send(_ text: String, effort: String = "balanced", plan: Bool = false, goal: Bool = false,
              modelKey: String = "auto", onSessionCreated: @escaping (ChatSession) -> Void) async {
        sendError = nil
