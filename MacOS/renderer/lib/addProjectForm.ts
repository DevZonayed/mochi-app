...
export type AddProjectTab = 'folder' | 'new' | 'clone' | 'context';
...
export const TABS: { id: AddProjectTab; label: string; sub: string }[] = [
  ...
  { id: 'context', label: 'Context',     sub: 'A universal operator with one continuous conversation' },
];

/** What "Context" needs to be enabled — just a sane name. The brain
    auto-creates the folder (~/Maestro/<name>-<rand>) + the single operator
    session, so there is nothing else to collect. */
export function validateContextInput(name: string): NewLocalValidation {
  const n = name.trim();
  if (!n) return { ok: false, reason: null };
  if (!/^[A-Za-z0-9 ._-]+$/.test(n)) return { ok: false, reason: 'Project name can use letters, numbers, spaces, dots, dashes and underscores.' };
  return { ok: true, reason: null };
