...
import { api, IS_WEBKIT, type Project, type ChatSession, type DirEntry, type CmdOutput } from './api';
...
export function RightSidebar({ project, session, changed, checks, onOpenFile, collapsed, onToggleCollapse }: {
  ...
  const loadRoot = React.useCallback(() => {
    ...
    // Scoped to the active session's branch worktree when it has one —
    // switching chats swaps the tree to that chat's branch checkout.
  ...
  React.useEffect(() => {
    ...
    // Auto-(re)load whenever the project OR the active session's worktree
    // scope changes.
    loadRoot();
  }, [loadRoot, project.id]);
