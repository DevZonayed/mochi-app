...
function RunsTab({ runs }: { runs: RunGroup[] }) {
  const [q, setQ] = React.useState('');
      ...
      {shown.map(g => (
        <div key={g.day} style={{ marginBottom: 20 }}>
          ...
          <div style={{ background: 'var(--bg-grouped)', borderRadius: 14, border: '0.5px solid var(--separator)', overflow: 'hidden', marginTop: 8, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
            {g.items.map((r, i) => (
              <div key={i} className="run-row" style={{ display: 'grid', gridTemplateColumns: '24px 1.3fr 1.6fr 1.2fr 0.7fr 0.7fr 56px', gap: 14, alignItems: 'center', padding: '13px 16px', borderBottom: i < g.items.length - 1 ? '0.5px solid var(--separator)' : 'none' }}>
  ...
  return (
    <div style={{ background: 'var(--bg-elevated)', borderRadius: 14, border: '0.5px solid var(--separator)', overflow: 'hidden', boxShadow: 'var(--card-shadow)' }}>
      <div style={{ padding: '44px 16px', textAlign: 'center' }}>
        <Icon name="shield" size={26} style={{ color: 'var(--ink-tertiary)', marginBottom: 12 }} />
        <div style={{ font: '700 var(--fs-title3)/1.2 var(--font-display)', color: 'var(--ink)', marginBottom: 6 }}>Audit log unavailable</div>
        <div style={{ font: '400 var(--fs-callout)/1.4 var(--font-text)', color: 'var(--ink-tertiary)' }}>This screen does not have a live audit-log source yet.</div>
      </div>
    </div>
  );
}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative', zIndex: 1 }}>
          ...
          <main style={{ flex: 1, overflowY: 'auto', padding: '24px 28px 36px' }}>
            ...
            <div key={tab} className="tab-fade">
              {tab === 'runs' ? <RunsTab runs={runs} /> : <AuditTab />}
            </div>
          </main>
        </div>
