...
export class GitService {
  ...
  async autoPushSession(session: ChatSession): Promise<{ ok: boolean; pushed?: boolean; reason?: string }> {
    ...
    const ref = (await localRefExistsAsync(dir, originRef)) ? originRef : (session.baseBranch ?? resolveBaseBranch(dir));
    if ((await aheadBehindAsync(dir, ref)).ahead === 0) return { ok: true, pushed: false, reason: 'nothing to push' };
