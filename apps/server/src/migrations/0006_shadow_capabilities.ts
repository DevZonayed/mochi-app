...
const SHADOW_CAPABILITY_LIMIT_0006 = 7;
 ...
 * non-empty array within the historical vocabulary size. App-level validation
 * (canonicalizeCapabilities) remains authoritative for known-enum + canonical sort.
 *
 * Historical migrations must not import the live shared capability vocabulary: a future
 * vocabulary append must be paired with an explicit additive DB migration.
 *
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`DO $$ BEGIN
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'shadow_req_caps_shape' AND conrelid = 'shadow_enrollment_request'::regclass
    ) THEN
      ALTER TABLE shadow_enrollment_request ADD CONSTRAINT shadow_req_caps_shape
        CHECK (jsonb_typeof(requested_capabilities) = 'array' AND jsonb_array_length(requested_capabilities) BETWEEN 1 AND ${sql.lit(SHADOW_CAPABILITY_LIMIT_0006)});
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'shadow_grant_caps_shape' AND conrelid = 'shadow_enrollment_grant'::regclass
    ) THEN
      ALTER TABLE shadow_enrollment_grant ADD CONSTRAINT shadow_grant_caps_shape
        CHECK (approved_capabilities IS NULL OR (jsonb_typeof(approved_capabilities) = 'array' AND jsonb_array_length(approved_capabilities) BETWEEN 1 AND ${sql.lit(SHADOW_CAPABILITY_LIMIT_0006)}));
    END IF;
  END $$`.execute(db);
}
