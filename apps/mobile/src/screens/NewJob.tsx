...
/** Static engine catalog. Status (available + count) is filled in from the
    live `/api/engine-status` + `/api/models` calls — the `cost: 0`
    placeholders were vestigial dummy data that didn't reflect reality. */
...
/** Per-engine live status surfaced in the picker — derived from
 *  /api/engine-status (available + reason) and /api/models (variant count). */
interface EngineMeta { available: boolean; reason: string; modelCount: number }
type EngineMetaMap = Record<ModelId, EngineMeta>;

/** Hook: fetch live engine status + model counts. Fails soft so the picker
 *  still renders if the call errors. */
...
/** Live engine status badge: green dot + "N models" or red dot + reason.
