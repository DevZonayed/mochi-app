...
export function ChatThread({ projectId, project, sessionId, onSessionCreated, onTurns, onOpenImage, flush, autoFocus }: {
  ...
  const sendComposedNow = React.useCallback(() => {
    ...
    setAttachments([]);
    if (streaming) {
      // Queue at the FRONT so the drainer picks it up first when the agent
      // finishes its current turn. The agent's in-flight answer survives.
      // (Images aren't supported on queued prompts today — they need
      // sendRaw's full payload — so if the user attached images, they fall
      // through to the explicit interrupt-and-send path. Text-only is the
      // common case and what the bug report covered.)
      if (imgs.length) {
        // Explicit interrupt is still the right call when there are images
        // — they couldn't ride the queue payload anyway.
