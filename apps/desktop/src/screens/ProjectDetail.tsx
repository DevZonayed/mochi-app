...
const IS_WRITE_TOOL = (name: string): boolean => /write|edit|create|patch|notebook/i.test(name);
...
/* A written-file chip: type badge + filename, with a hover preview of the
   content the agent wrote (rendered through a portal so nothing clips it). */
function FileChip({ path, preview }: { path: string; preview?: string }) {
