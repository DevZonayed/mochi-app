...
async function runClaude(
  ...
  const maestroServer = ((imageGen || skillsCtx || bgCtx || browserCtx || scheduleCtx || gitCtx) && !plan)
    ? createSdkMcpServer({
        name: 'maestro',
        version: '1.0.0',
        tools: [
...
export class LocalEngine {
  ...
  async run(jobId: string, opts: { effort?: Effort; engine?: EngineId; model?: string; reviewer?: RoleChoice | 'off'; plan?: boolean; goal?: boolean; browser?: boolean } = {}): Promise<Job> {
    ...
    try {
      ...
      // remote, mount the pr_*/git_* tools AND inject the PR_DIRECTIVE so the
      // agent knows when to call them. Off in plan mode (the lifecycle is
      ...
      const runPrimary = (): Promise<EngineRun> => master === 'claude'
        ? runClaude(prompt, cwd, effort, anthropicKey, goalMode ? GOAL_MAX_TURNS : undefined, hooks, resumeId, masterModel, opts.plan, this.imageGen, job.projectId, claudeImages, skillsCtx, bgCtx, browserCtx, claudeCustomMcp, scheduleCtx, gitCtx)
      ...
      if (master === 'claude' && main.hitMaxTurns && !main.hitLimit && !opts.plan && !ac.signal.aborted) {
        ...
        while (main.hitMaxTurns && main.sdkSessionId && turnsSoFar < ceiling && !ac.signal.aborted) {
          ...
          let cont: EngineRun;
          try {
            cont = await runClaude(CONTINUE_PROMPT, cwd, effort, anthropicKey, undefined, contHooks, main.sdkSessionId, masterModel, false, this.imageGen, job.projectId, undefined, skillsCtx, bgCtx, browserCtx, claudeCustomMcp, scheduleCtx, gitCtx);
